...
\textit{This dissertation aims to layout the design and implementation work done to provide the SPIIR pipeline with the ability to support any number of detectors without hindering the functionality of the pipeline (\(N\)-detectors).}
...
This means that whilst we cannot use PyCBC when considering how to support any number of detectors within a gravitational-wave searching algorithm nor its outputs, we can use PyCBC to consider a programming interface with which to support other detectors.
\\
...
This means that the process of adding support for detectors involves changing a number of different files, as well as modifying several internal data structures \cite{messick} \textemdash{} which means that GstLAL does not support \(N\)-detectors.
\\
...
\begin{table}[t]
    ...
    \end{tabular}
    \caption{An evaluation of the implementation of \(N\)-detector against the criteria specified in section \ref{sec:design_process:criteria}}
    \label{tab:eval}
\end{table}
...
As per the criteria laid out in section \ref{sec:design_process:criteria}, the implementation of \(N\)-detectors has been successful, however there are still areas for improvement.
...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    \caption{Simplified callgraph for coherent post-processing in the SPIIR pipeline.\\ Function calls are represented by solid arrows, the ordering of the calls is represented by dashed arrows.\\ GPU functions are in green boxes, and CPU functions are in red boxes.}
    \label{fig:callgraph}
\end{figure}
