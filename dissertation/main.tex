...
\begin{center}
    \begin{minipage}[t]{0.6\textwidth}
        ...
        A solution proposed by this dissertation reduced the development cost of supporting additional detectors in the SPIIR pipeline to just 9 lines of code across 2 files by introducing a method of mapping combinations of detectors to integers based on powers of two and replacing individual detector variables with C arrays.
...
At the time of the start of this research, the SPIIR pipeline supported the use of three detectors for gravitational wave detection \textemdash{} the two American LIGO detectors and the Italian Virgo detector \textemdash{} and additional detectors are planned to be introduced soon.
This presented several issues with the existing pipeline design.
...
As the number of available detectors continues to grow, development work that could be spent on improving the optimisation, precision, or accuracy of the pipeline would instead have to be spent allowing for those detectors to be used.
\\
...
Finally, section \ref{sec:further_work} will provide some suggestions for future research that can be based on this work.
...
GstLAL is a gravitational-wave detection library, that exposes components of LALSuite \cite{lalsuite} as GStreamer elements for use in other analysis pipelines \textemdash{} including the SPIIR pipeline \textemdash{} as well as providing its own pipeline for processing raw signals from detectors into lists of gravitational wave candidates \cite{gstlal_docs}.
...
Warps are the smallest unit that GPUs schedule, and all threads in a warp must execute the same instruction \textemdash{} although each thread maintains its own instruction pointer and can branch independently from the warp at a small performance cost.
...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    \caption{An example of calculating work for a merge-sort like algorithm.}
    Summed nodes are in red.
    \label{fig:work}
\end{figure}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        \draw[->] (centerl.east) -- (right.west);
    \end{tikzpicture}
    \caption{An example of calculating span for a merge-sort like algorithm.}
    Summed nodes are in red.
    \label{fig:span}
\end{figure}
...
Another helpful theorem for analysis is \textit{\textbf{Brent's Theorem}}, which states that for an algorithm that can run in parallel on \(N\) processors, the same algorithm can be executed on \(P < N\) processors in a time of approximately \cite{BrentsLaw}:
...
\begin{itemize}
    ...
    \item \textbf{Individual detectors should still be exposed in Python code}\\
        ...
        First, exposing individual detectors in the Python code allows for easier programming debugging of any values that may be incorrect, as the values of each detector would be immediately evident by the variable names instead of having to be indirectly referenced by knowledge of the data structure used to hold the group of detectors.
...
\begin{itemize}
    ...
    \item \textbf{LIGO DataGrid (LDG) \cite{ldg}}\\
        ...
        Testing and benchmarking were performed on the LDG as the available nodes there closely mirror those that the SPIIR pipeline will run on.
...
The relevant code sections that may need to be modified to support any number of detectors are in any step after the combination of the individual detector SPIIR filtering, as observed in Figure \ref{fig:spiir_structure}.
This includes:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item Adding new detectors should be able to be done by a non-technical individual
        \begin{enumerate}
            \item Maximum number of required edited files: \(\leq 3\)
        \end{enumerate}
...
Section \ref{sec:final_design:patches} discusses the major components and changes that each source code patch made to the codebase, and the testing that the patches went through will be described in section \ref{sec:final_design:testing}.
...
\( (x + y)^n = \sum^n_{k=0}\binom{n}{k}x^ky^{n-k} \) when \(x = y = 1\), it can be noted that \cite{CLRS_combinations};
...
This lends itself well to binary numbers, as an 8-bit binary number can represent \(2^8\) numbers or all the combinations of 8 elements \textemdash{} which means that all combinations of \(N\) detectors can be represented using \(N\) bits.
...
\begin{table}[t]
    ...
    \begin{tabular}{lc}
        ...
        Maximum number of required edited files & {\unicodefont ✔} (2) \\
...
\begin{enumerate}
    ...
    \item \textbf{The output table format should be unchanged} \\
        Whilst the table sent to GraceDB was unchanged, the files output by the pipeline had their header names modified as the full name of the detector was appended to detector specific variables instead of a single representative letter.
    \item \textbf{Adding new detectors should be able to be done by a non-technical individual} \\
        This is subjective, but arguably achieved.
        \begin{enumerate}
            \item \textbf{Maximum number of required edited files: \(\leq3\)} \\
...
Thus, it can be estimated that the number of files that would have to change would be 16 files, with a total number of lines changed at approximately 1000.

In comparison, as stated in section \ref{sec:final_design:evaluation}, the number of files that would need to change to support an additional detector using this system is only 2, representing a large leap forward in ease of modification.
...
This work, however, does continue to allow the SPIIR pipeline to support the use of any subset of supported detectors for gravitational-wave searching, representing a significant continuing advantage to the SPIIR pipeline over PyCBC.

\paragraph{GstLAL}

The implementation to easily support any number of detectors in SPIIR makes significant improvements to the existing support in GstLAL.
The most recent detector addition (GEO600, June 2020) involved modifying 9 files with 187 new lines of code \textemdash{} and the detector addition did not include being able to get the data from the detector, just support for use in the coincidence search algorithm \cite{messick}.
In comparison, the equivalent change in the SPIIR pipeline would modify only one file (as getting data from the detector is not required) and would involve adding approximately 8 lines of code, a significant improvement.

\subsection{A complexity analysis of the parallel post-processing of the SPIIR pipeline} \label{sec:discuss:analysis}

As discussed in \ref{sec:lit_review:par_comp_analysis}, the SPIIR pipeline was designed to be as low latency as possible, and an asymptotic complexity analysis of components that may be impacted by the addition of new detectors allows for the measurements of the potential runtime cost of doing so.
...
\begin{figure}[t]
    \centering
    \begin{tikzpicture}[
        cpu/.style={rectangle, text=black, draw=red!60, fill=red!5, very thick, minimum size=7mm},
        gpu/.style={rectangle, text=black, draw=green!60, fill=green!5, very thick, minimum size=7mm},
]
        \node[cpu] (cudapostcohprocess) {\texttt{cuda\_postcoh\_process}};
        \node[cpu] (peaksoverthresh) [below left=of cudapostcohprocess] {\texttt{peaks\_over\_thresh}};
        \node[cpu] (transposesnglsnr) [below=of cudapostcohprocess] {\texttt{transpose\_snglsnr}};
        \node[gpu] (transposematrix) [below left=of transposesnglsnr] {\texttt{transpose\_matrix}};
        \node[cpu] (cohsnrandchisq) [below right=of cudapostcohprocess] {\texttt{cohsnr\_and\_chisq}};
        \node[gpu] (kercohmax) [below left=of cohsnrandchisq, right=of transposematrix] {\texttt{ker\_coh\_max\_and\_chisq\_versatile}};
        \node[gpu] (kercohskymap) [below=of cohsnrandchisq, right=of kercohmax] {\texttt{ker\_coh\_skymap}};

        \draw[->] (cudapostcohprocess.south) -- (peaksoverthresh.north);
        \draw[->] (cudapostcohprocess.south) -- (transposesnglsnr.north);
        \draw[->] (cudapostcohprocess.south) -- (cohsnrandchisq.north);
        \draw[->] (transposesnglsnr.south) -- (transposematrix.north);
        \draw[->] (cohsnrandchisq.south) -- (kercohmax.north);
        \draw[->] (cohsnrandchisq.south) -- (kercohskymap.north);
        \draw[->, dashed] (peaksoverthresh.east) -- (transposesnglsnr.west);
        \draw[->, dashed] (transposesnglsnr.east) -- (cohsnrandchisq.west);
        \draw[->, dashed] (transposematrix.east) -- (kercohmax.west);
        \draw[->, dashed] (kercohmax.east) -- (kercohskymap.west);
    \end{tikzpicture}
    \caption{Simplified callgraph for coherent post-processing in the SPIIR pipeline.}
    Function calls are represented by solid arrows, the ordering of the calls is represented by dashed arrows.\\ GPU functions are in green boxes, and CPU functions are in red boxes.
    \label{fig:callgraph}
\end{figure}
...
As can be noted from our own analysis, the process of reduction can be parallelised by the use of a binary tree of operations, where each vertex in the binary tree combines the results of the two parent vertices.
In the case of determining the maximum of two numbers, each vertex is identical in the amount of work done, and thus each vertex is a single unit of work.
As there are \(N\) elements in the original array, it can be noted that the height of the binary tree is \(\log N\), and each level of the binary tree has \(N_l/2\) vertices, thus the total number of vertices in the binary tree is \(\sum_{i=0}^{\log N}{2\times{i}} = N\).
Using this information, it can be determined that the \textit{\textbf{work}} of a parallelised reduction is \(T_1 = O(N\times1) = O(N)\), and that the \textit{\textbf{span}} of the reduction is \(T_\infty = O(\log N \times {1}) = O(\log N)\).
...
Using the span and work laws, it can be observed that any algorithm using the above method is bounded by the inequalities \(O(\log N) \leq T_P, \dfrac{O(N)}{P} \leq T_P\).
This means that best possible time complexity with \(P\) processors is \(O(\log N)\) (equation \ref{eq:span_law}).
The minimum number of processors required to achieve this runtime can be determined using the formula \(T_P = O(\log N) = \dfrac{O(N)}{P}\), which can be rearranged to
\[
    P = \dfrac{N}{\log N},
\]
thus the time complexity cannot improve past \(P = N/\log N\) processors.
It can also be observed that using \(P = N/\log N\) processors gives a \textit{\textbf{cost}} of \(C_P = N\), which is identical to the sequential algorithm.
\\
...
The function that is used for determining the number of samples over the threshold (\texttt{peaks\_over\_thresh}) is a sequential algorithm that runs on the CPU, and is analysed as such, although there is an alternative GPU-based implementation that is not used.
...
Recalling from section \ref{sec:discuss:analysis:reduce} that for maximum element reduction \(T_1 = O(N)\), and that this operation is performed \(N\) times, where \(N\) is the number of samples, it can determined that this initial reduction has a time complexity of \(O(NT)\), where \(T\) is the number of templates.

The function then determines the maximum SNR across the templates found from the previous step by stepping through every combination of samples and removing SNR samples that are using the same template and have a lower SNR, resulting in a step with a time complexity of \(O(N^2)\).

The function then calculates the maximum overall SNR for the input samples (\(O(N)\)) and cycles through every maximum SNR to cluster maxima that are close together to be a single combined maximum.
The number of maxima is bounded by (\(O(\min\{N,T\})\)), as there cannot be more maxima than there are samples or templates.

This gives the overall function a time complexity of \(O(NT + S^2 + S + \min\{N,T\})\), which can be reduced to the dominating terms of:
...
Using this observation, it can be observed that the \textit{\textbf{span}} of the algorithm is \(T_\infty = (32\times8)\times4 = O(1024) = O(1)\), and the \textit{\textbf{work}} is \(T_1 = O(NT)\), where \(N\) is the number of samples and \(T\) is the number of templates.
Thus, the \textit{\textbf{parallelism}} of the transpose is \(p = NT\).

Using the span and work laws (equations \ref{eq:span_law} and \ref{eq:work_law}), it can be shown that the above method is bounded by the inequalities \(O(1) \leq T_P, \dfrac{O(NT)}{P} \leq T_P\).
...
Thus, whilst the asymptotic time complexity does not change, the number of processors available is smaller, so the actual runtime of each loop is approximately the warp size slower.
...
Conversely, the overall parallelism cannot be determined as the post-processing step spans a number individual functions that can each be run with a different set of processors.
As the step to determine the number of peaks over a threshold (see section \ref{sec:discuss:analysis:peaks}) is sequential, its time complexity can be considered to be contributing to both the span and work of the total pipeline.
...
With this in mind, the \textit{\textbf{span}} of the post-processing can be calculated to be:
...
\end{equation}
where \(D\) is the number of detectors, \(S\) is the number of sky directions, \(T\) is the number of templates, \(N\) is the number of samples and \(P = \min\{ N, T \}\).
...
\end{equation}
where \(D\) is the number of detectors, \(S\) is the number of sky directions, \(T\) is the number of templates, \(N\) is the number of samples, \(B\) is the number of is the number of times shifts made to background noise and \(P = \min\{ N, T \}\)
...
As equation \ref{eq:total_span} shows, even with the use of an infinite number of processors there are still terms that scale cubically with the number of detectors.
Therefore, it can be assumed that the real runtime of the coherent post-processing will increase in a cubic manner relative to the number of detectors.

As such, it would be the recommendation of the author that further work is done to mitigate the increased latency that would be introduced by the addition of new detectors.

\section{Future work} \label{sec:further_work}

The change to C arrays from individual detector variables in section \ref{sec:final_design:patches:ndetector} uses fixed-size arrays, which means that the memory usage of the pipeline will be invariant to the number of detectors being used, and will instead depend on the number of detectors supported.
This may present memory issues with a large number of supported detectors as GPU memory is generally at a premium and should be conserved if possible.
...
One method that could be employed to counter the potential runtime cost of adding new detectors \textemdash{} as discussed in section \ref{sec:discuss:analysis:implications} \textemdash{} is to have some detectors skip the coherent search step.
The detectors skipping this step could be reintroduced later in the pipeline to help improve the accuracy of the generated skymaps whilst not causing a noticeable impairment of performance.
...
By implementing both the mapping combinations of detectors to integers based on powers of two and by removing individual detector variables by replacing them with C arrays, the development cost of supporting additional detectors has been reduced to be able to be completed by even non-technical individuals, with only two files requiring modifications.
...
\begin{appendices}
    ...
    \subsection{Making the combinations in \texttt{IFOComboMap} follow sums of powers of two} \label{app:patches:icombo}
    \linespread{0.3}
